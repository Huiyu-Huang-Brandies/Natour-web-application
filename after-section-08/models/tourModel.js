...
const tourSchema = new mongoose.Schema(
  {
    ...
    ratingsAverage: {
      ...
      set: val => Math.round(val * 10) / 10 // Math.round give integer
    },
    ratingsQuantity: {
      type: Number,
      default: 0
...
tourSchema.pre(/^find/, function(next) {
  this.populate({
    path: 'guides',
    select: '-__v -passwordChangedAt' // -: minus the things you don't want to display
  });
  next();
});

// AGGREGATION MIDDLEWARE
// add this match stage before all the other stages(but not before the geoNear)
